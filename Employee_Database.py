...
def GO():
	...
	conn = sqlite3.connect('/home/mirafra/ali/tk/mini_pcj/employee.db')
	cur=conn.cursor()
	cur.execute("SELECT * FROM EMPLOYEE WHERE mobile=='%s';" % my_mobile)
	data=cur.fetchall()
	cur.close()
	conn.close()
...
def save():
	...
	for t in [(emp_name,p_domain,s_domain,mobile,email_id,cctc,ectc,notice,d_of_d,cur_loc,pref_loc),]:
		conn.execute('insert into EMPLOYEE values(?,?,?,?,?,?,?,?,?,?,?)',t)
		conn.commit()
...
window.geometry('900x850+120+120')
#----------------------------------------------------- Labels creation -------------------
...
#===================================================== buttons ====================================
btn=Button(window,text='SAVE',width=15,command=save)
btn.grid(row=11,column=0,pady=20)

btn=Button(window,text='CLOSE',width=15,command=window.destroy)
...
btn=Button(window,text='GO',width=5,command=GO)
